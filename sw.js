...
const CACHE_NAME = 'travel-go-v10.3.1';
const APP_SHELL = [
  './',
  './index.html',
  './manifest.json',
...
<body><div class="card"><h1>目前離線中</h1><p>若你之前已成功連網開啟過一次 Travel Go!!!，主程式與已儲存資料仍可使用。</p><p>天氣、翻譯、即時匯率等需要網路的功能，恢復連線後會自動正常。</p></div></body></html>`;
...
async function precacheOne(cache, url) {
  try {
    const request = new Request(url, {
      mode: isCrossOrigin(url) ? 'no-cors' : 'same-origin',
    ...
    if (response && (response.ok || response.type === 'opaque')) {
      ...
      console.log('SW cached:', url, response.type || response.status);
      return;
    }
    console.warn('SW skipped non-cacheable response:', url, response && response.status);
  } catch (err) {
...
self.addEventListener('install', event => {
  ...
  event.waitUntil((async () => {
    ...
    await Promise.all(APP_SHELL.map(url => precacheOne(cache, url)));
  })());
});
...
self.addEventListener('fetch', event => {
  ...
  if (request.mode === 'navigate') {
    event.respondWith((async () => {
      ...
      try {
        ...
        return (await caches.match(request)) || cachedIndex || new Response(OFFLINE_HTML, { headers: { 'Content-Type': 'text/html; charset=UTF-8' } });
      }
    })());
    return;
  }
  ...
  event.respondWith((async () => {
    const cached = await caches.match(request) || await caches.match(url);
    if (cached) return cached;
